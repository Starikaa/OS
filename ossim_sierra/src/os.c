...
void segfault_handler(int signal) {
    fprintf(stderr, "\n[ERROR] Segmentation fault detected! Exiting...\n");
    fflush(stderr);  // Ensure log is written before exit
    exit(1);
}
...
int main(int argc, char *argv[]) {
    ...
    // 🔹 Ensure immediate flushing of output
    setbuf(stdout, NULL);
    setbuf(stderr, NULL);
    ...
    /* Create all MEM SWAP */
    for (int sit = 0; sit < PAGING_MAX_MMSWP; sit++)
        init_memphy(&mswp[sit], memswpsz[sit], rdmflag);
